...
def fetch_live_data():
    ...
    if r.status_code == 200:
        ...
        df_long.to_csv("air_quality_data_longer.csv", index=False)
        return df_long
...
app.layout = html.Div([
    ...
    dcc.DatePickerRange(
        ...
        max_date_allowed=df_long["datetimeLocal"].max().date(),
        min_date_allowed=df_long["datetimeLocal"].min().date(),
        start_date=df_long["datetimeLocal"].min().date(),
        end_date=df_long["datetimeLocal"].max().date(),
        initial_visible_month=df_long["datetimeLocal"].min().date()
    ),
    dcc.Dropdown(
        id="pollutant-dropdown",
        options=[{"label":i.upper(),"value":i} for i in df_long["parameter"].unique()],
